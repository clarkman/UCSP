...
for s = 1 : numel(srcs)
  ...
  peaks = getAudioRMS( srcExps, sens );  sz = size(peaks);
  ...
  % end
  plot(xAx,20.*log10(peaks)+140,'Marker','o','LineStyle','none','Color',colrs(mod(s,szc(1))+1,:));
  numLeg = numLeg + 1;
...
ylabel('Peak dB')
setPlotSize();
title( sprintf('Audio rms vs. source') )
writeJpegFile( outBaseDir, 'Accelerometer-source-rms.jpg' )
