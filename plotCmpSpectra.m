function plotCmpSpectra( arr, sens, inds, beg, fin, Fs )
...
realExps{1} = extractRows( exps, idxs{1} );
realExps{2} = extractRows( exps, idxs{2} );
realExps{3} = extractRows( exps, idxs{3} );
realExps{4} = extractRows( exps, idxs{4} );

expCounts = zeros(4,2);
expCounts(1,1) = numel(idxs{1}); expCounts(1,2) = 1;
expCounts(2,1) = numel(idxs{2}); expCounts(2,2) = 2;
expCounts(3,1) = numel(idxs{3}); expCounts(3,2) = 3;
expCounts(4,1) = numel(idxs{4}); expCounts(4,2) = 4;

expCounts = sortrows( expCounts, -1 );

primary = realExps{expCounts(1,2)};
sz = size(primary);
numPrExps = sz(1);
...
for e = 1 : numPrExps
    ...
    for set = 1 : 4
    	expsGrp = realExps{expCounts(e,2)};
        extant = find( primary(e,13) == expsGrp(:,13) )
    end
    continue
