function compareAudio( exps, sens )
...
% % Rev4
% rawInds = find( casExps(:,1) == 2 & casExps(:,3)~=-9999 & casExps(:,4)==205 );
% Fs = 24000;
% begSamp = Fs;
% finSamp = Fs + 2*Fs/10;
% plotSpectra( casExps, sens, rawInds, begSamp, finSamp, Fs );


% % RevA
% rawInds = find( casExps(:,1) == 2 & casExps(:,3)~=-9999 & casExps(:,4)>500 & casExps(:,4)<2000 & casExps(:,6)==0 & casExps(:,7)==4.5 );
% Fs = 24000;
% begSamp = Fs;
% finSamp = Fs + 2*Fs/10;
% plotSpectra( casExps, sens, rawInds, begSamp, finSamp, Fs );
...
% Knowles
rawInds = find( casExps(:,1) == 12 & ( casExps(:,4)==9997 | casExps(:,4)==9998 ) )
Fs = 48000;
begSamp = Fs;
finSamp = Fs + 2*Fs/10;
plotSpectra( casExps, sens, rawInds, begSamp, finSamp, Fs );
