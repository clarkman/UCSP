...
display( [ 'Computing pulse azimuths for site: ', seg1.DataCommon.station, ' CHANNEL', chan, ' ', datenum2str(seg1.DataCommon.UTCref) ] );
...
sz = size( pulses );
numPulses = sz(1);
display( sprintf( 'Counted %d pulse Azs', numPulses ) );
...
if( numPulses > 0 )
    ...
    for pul = 1 : numPulses
        ...
        nextRow = [ pulses(pul,1), pulses( pul, 2 ), pulses(pul,3), pulses(pul,4), double(pLen), double(v1), double(v2), double(v3), mean1, mean2, mean3, median1, median2, median3, double(iPeak1), double(iPeak2), double(iPeak3), std1, std2, std3, now ];
