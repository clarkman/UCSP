...
% end
% % Get plot colors from mySQL database ...
% chColor{1} = getRGBcolor( procDir, 'ch1TimeSeries' );
% chColor{2} = getRGBcolor( procDir, 'ch2TimeSeries' );
% chColor{3} = getRGBcolor( procDir, 'ch3TimeSeries' );
%chColor{4} = getRGBcolor( procDir, 'ch4TimeSeries' );
...
for iobj = 1 : numobjs
    ...
    % [ch, chan] = makeCh( obj.DataCommon.channel );
    % if chan > 4
    %   chan = 3; % Blue
    % end
    ...
    %line(obj.samples(:,1)+obj.timeOffset, obj.samples(:,2), 'Marker', symbol, 'Color', chColor{chan});
    line(obj.samples(:,1)+obj.timeOffset, obj.samples(:,2), 'Marker', symbol);
    else
    ...
    plot(axisvec, obj.samples, 'Marker', symbol);
    end
...
set(gca,'XLim',[UTCref+ timeZoneOff, UTClast+ timeZoneOff]);
%set(gca,'XLim',[UTCref UTClast]);
unds = strfind( titlestr, '_' );
for u = 1 : length(unds)
  titlestr(unds(u)) = ' ';
end
