function plotAudioDelay( exps, sens, beg )
...
for s = 1 : numel(srcs)
  ...
  hold on;
  peaks = getAudioPeakOnset( srcExps, sens );   sz = size(peaks)
  if doOnset
...
if doOnset
  title( sprintf('NMHS Tests, Audio peak onset delay vs. source') )
  %writeJpegFile( outBaseDir, 'Piezo-source-msec.jpg' )
  writeJpegFile( outBaseDir, 'Audio-source-msec.jpg' )
  ylabel('msec (after 1 sec)')
else
  %legend(leg)
  title( sprintf('NMHS Tests, Audio peak onset delay vs. Casella') )
  writeJpegFile( outBaseDir, 'Audio-source-msec-casella.jpg' )
  ylabel('msec (after 1 sec)')
end
return
